...
import { findBosses, setBossTopSpecs } from "../boss.model";
import {
  setCharacterBossRankings,
  ...
  for (const realm of realms) {
    ...
    for (const metric of Object.values(METRIC_TYPE)) {
      ...
      for (const boss of await findBosses({ realm: realm.name })) {
        if (
          Array.isArray(options.bossIds) &&
          options.bossIds.includes(boss.remote_id) === false
        ) {
          continue;
        }
        ...
        for (const difficulty of diffs) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            statsByGuid[guid]![bossRemoteId]![mode] = {
              ...item,
              rank: overallRank,
            };
            overallRank++;
            ...
            for (const [specStr, items] of Object.entries(topSpecs)) {
              ...
              for (let i = 0; i < len; i++) {
                ...
                if (bk) {
                  ...
                  statsByGuidBySpec[spec]![guid]![bossRemoteId]![mode] =
                    payload;
