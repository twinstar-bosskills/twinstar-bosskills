...
export const getRaidLockStats = async (
  args: GetRaidLockStatsArgs,
): Promise<RaidLockStats> => {
  const fallback = async () => {
    ...
    const topKeyValue = (item: RaidLockData): TopKeyValue => {
      ...
      const topDay =
        byWeekDay.slice().sort((a, b) => b.value - a.value)[0] ?? null;
      ...
      const topHour =
        byHour.slice().sort((a, b) => b.value - a.value)[0] ?? null;
      return {
        byWeekDay,
        topDay,
        byHour,
    ...
    const toStats = (
      data: Awaited<ReturnType<typeof findBossKills>>,
    ): RaidLockData => {
      ...
      const wipePercentage =
        kills > 0 ? Math.round(10000 * (wipes / kills)) / 100 : 0;
      ...
      const item = {
        ...
        killsByBoss: Object.values(killsByBoss).sort(
          (a, b) => b.count - a.count,
        ),
        wipesByBoss: Object.values(wipesByBoss).sort(
          (a, b) => b.count - a.count,
        ),
        kills,
        wipes,
