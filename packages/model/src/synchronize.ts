...
import { safeGC } from "@twinstar-bosskills/core/dist/gc";
...
export const synchronize = async ({
  ...
  const bossesByRaidId = bosses.reduce(
    (acc, boss) => {
      acc[boss.raid_id] ??= [];
    ...
    for (const boss of bossesByRaidId[raid.id] ?? []) {
      ...
      const promises = bosskills.map((bk, i) => {
        ...
        const work = async () => {
          ...
          const detail = await getBossKillDetail({
            realm: realmEnt.name,
            id: bk.id,
          });
          if (detail) {
            onLog(
              `${xOfY} Processing ${boss.name} bosskill detail ${detail.id}`,
            );

            if (Array.isArray(detail.boss_kills_players)) {
              onLog(
                `${xOfY} Processing ${boss.name} bosskill detail ${detail.id} - players`,
              );
            ...
            if (Array.isArray(detail.boss_kills_loot)) {
              onLog(
                `${xOfY} Processing ${boss.name} bosskill detail ${detail.id} - loot`,
              );
            ...
            if (Array.isArray(detail.boss_kills_deaths)) {
              onLog(
                `${xOfY} Processing ${boss.name} bosskill detail ${detail.id} - deaths`,
              );
            ...
            if (Array.isArray(detail.boss_kills_maps)) {
              onLog(
                `${xOfY} Processing ${boss.name} bosskill detail ${detail.id} - timeline`,
              );
          ...
          onLog(
            `${xOfY} Processing ${boss.name} bosskill: ${bk.id} took ${took}`,
          );
          timeSum += took;
        };
        return work()
...
const getOrCreateBoss = async ({
  ...
  const result = await db
    ...
    .where(({ eb }) => {
      return eb.and([
        eb("boss.remote_id", "=", boss.entry),
        eb("boss.raid_id", "=", raidId),
      ]);
    })
    .executeTakeFirst();
  let ent = result ?? null;
...
const getOrCreateBosskill = async ({
  bossId,
  realmId,
  raidId,
  bk,
...
const createBossKillDeaths = async ({
  bosskillId,
  deaths,
}: BossKillDeathsArgs) => {
  if (deaths.length === 0) {
...
const createBossKillTimeline = async ({
  bosskillId,
  timeline,
}: BossKillTimelineArgs) => {
