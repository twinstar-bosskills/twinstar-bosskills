...
export const getRanks = async (args: GetRanksArgs): Promise<GetRanksResult> => {
  const fallback = async () => {
    ...
    const raids = (await getRaids({ realm })).reduce(
      (acc, raid) => {
        acc[raid.id] = raid;
        return acc;
      },
    ...
    for (const boss of bosses) {
      ...
      data.ranks[raid.id]![boss.id]!.sort((a, b) => {
        if (metric === METRIC_TYPE.DPS) {
          return characterDps(a.characters[0]!) < characterDps(b.characters[0]!)
            ? 1
            : -1;
        }
        return characterHps(a.characters[0]!) < characterHps(b.characters[0]!)
          ? 1
          : -1;
      });
    }
