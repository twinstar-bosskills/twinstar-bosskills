...
<QuadTree dataset={dataset || $data} y="x" let:x let:y let:visible let:found let:e>
	...
	{@const hasFoundSorted0 = typeof foundSorted[0]?.value !== 'undefined'}
	{#if visible === true}
		<div style="left:{(x / 100) * $width}px;" class="line" />
		<div
			class="tooltip"
			style="
          width:{w}px;
          display: {visible ? 'block' : 'none'};
          top:calc({hasFoundSorted0 ? $yScale(foundSorted[0]?.value) : 0}% + {offset}px);
