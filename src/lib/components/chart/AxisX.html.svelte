<script lang="ts">
	...
	export let formatTick = (d: any, i: number): string => d;

	/** @type {Number|Array|Function} [ticks] - If this is a number, it passes that along to the [d3Scale.ticks](https://github.com/d3/d3-scale) function. If this is an array, hardcodes the ticks to those values. If it's a function, passes along the default tick values and expects an array of tick values in return. If nothing, it uses the default ticks supplied by the D3 function. */
	export let ticks: number | unknown[] | ((v: unknown) => unknown) | undefined = undefined;
...
<div class="axis x-axis" class:snapTicks>
	{#each tickVals as tick, i (tick)}
		...
		>
			<div class="text" style="top:{yTick}px;">{formatTick(tick, i)}</div>
		</div>
	{/each}
	{#if baseline === true}
